...
const CreateProfessor: FC<AbstractEmptyInterface> = (props: any) => {
  ...
  const onSubmit = (e: any) => {
    ...
    if (!professorStore.selectedProfessor) {
      ...
      props.professorStore.updateProfessor(professor).then((editProf: any) => {
        ...
        // if (editUser?.status === 200  && userStore.filters?.currentlyWorking === false) {

        //   history.push("/user/list");
        //   userStore.getFilteredUserArchive({ filter: "" });
        // }
  ...
  const handleOpenConfirmModal = (path: string) => (e: any) => {
    e.preventDefault();
    // const newuser = {
    //   ...professor, isArchive: false
    // }

    // if (professorStore.selectedProfessor) {
    //   props.professorStore.updateProfessor(newuser).then((editProf: any) => {
