...
const ListStudent: FC<AbstractEmptyInterface> = (props: any) => {
  ...
  const searchFilter = (searchField: any) => {
    ...
    for (let i = 0; i < listStudents.length; i++) {
      if (searchField === listStudents[i].firstName ||
        searchField === listStudents[i].lastName ||
        searchField === listStudents[i].height
      ) {

        studentStore.getFilteredStudent({
          filter: listStudents[i].firstName ||
            listStudents[i].lastName ||
            listStudents[i].height
        });

        // studentStore.setOpenPDF(false);
        setOpen(false);
      }
